"""
Shared configuration untuk semua crawler (cnnindonesia/detik/kompas/youtubecomment).

Daftar query dipindah & disintesis dari kompas/keywords.py (daftar paling
matang & terkategorisasi di antara semua crawler) - kata kunci di 3 crawler
lain sudah tercakup secara semantik di sini, jadi ini jadi satu-satunya
sumber kebenaran untuk query pencarian.

Format KEYWORDS: (query, target)
...
# Lookup cepat query -> target, dipakai crawler yang menandai target setelah fetch.
KEYWORD_TARGET_MAP = dict(KEYWORDS)

# Rentang tahun crawling yang dipakai bareng semua sumber.
START_YEAR = 2021
END_YEAR = 2026
